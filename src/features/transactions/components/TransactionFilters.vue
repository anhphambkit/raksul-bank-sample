...
const props = defineProps<{ filters: TransactionFiltersValue; accounts: Account[] }>()
...
<template>
  <form
    ...
    class="rounded-xl border border-default bg-white p-4 sm:p-5"
    @submit.prevent="apply"
  >
    <div class="grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-6">
      <div class="min-w-0 sm:col-span-2 xl:col-span-3">
        <label for="transaction-search" class="mb-1.5 block text-sm font-medium text-highlighted"
      ...
      <div class="min-w-0 sm:col-span-2 xl:col-span-3">
        <label for="transaction-account" class="mb-1.5 block text-sm font-medium text-highlighted"
          >Account</label
        >
        <USelect
      ...
      <div class="grid min-w-0 grid-cols-2 gap-3 sm:col-span-2 xl:col-span-3">
        <div class="min-w-0">
          ...
          <UInput
            ...
            size="lg"
            class="w-full"
            :aria-describedby="error ? 'transaction-filter-error' : undefined"
          />
        </div>
        <div class="min-w-0">
          <label for="transaction-date-to" class="mb-1.5 block text-sm font-medium text-highlighted"
            >To (UTC)</label
          >
          <UInput
    ...
    <p v-if="error" id="transaction-filter-error" role="alert" class="mt-3 text-sm text-error">
      {{ error }}
    </p>
    <div class="mt-4 flex flex-wrap items-center gap-3">
      <UButton type="submit" class="min-h-11" icon="i-lucide-list-filter">Apply filters</UButton>
