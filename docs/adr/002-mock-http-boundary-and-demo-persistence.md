...
IndexedDB serializes writes across connections to the same object store, but browser storage is local, user-editable and subject to browser-dependent durability/retention. After commit, a storage-event token prompts same-origin demo tabs to cancel stale queries and refetch. Reads do not broadcast, and notification failure cannot undo a committed operation. There is no cross-device persistence (explicitly deferred). Mock ownership filtering is not server-side authorization. A real integration must supply authentication, authorization, runtime contract validation and transactional persistence.
