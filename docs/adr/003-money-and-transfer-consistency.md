...
Within one IndexedDB readwrite transaction, resolve or create the beneficiary, debit the source, credit an own/internal recipient when applicable, add linked activity and store the completed transfer. External recipients have a stored identity snapshot and a source debit only. Review performs no mutation. Resolve success only after commit; rejection or abort leaves the previous committed state—including beneficiaries—intact.
