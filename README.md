...
In demo mode, entering **Someone else** prefills an unsaved same-bank recipient (Jordan Lee, `200000008319`). Switching to **Other bank** prefills Maple Apartments at Harbor Bank (`300000002106`). These fields remain editable; restored drafts take precedence, amounts are not prefilled, and backend mode does not use these defaults. Same-bank entry still requires **Check account**.
