...
Open **Transfer**, choose an active source, then **My accounts** or **Someone else**. Select or enter a destination, enter a plain USD decimal amount and an optional reference (up to 140 characters), then select **Review transfer**. Review is read-only and displays masked account numbers, recipient/bank, balance, currency, amount and reference. Only **Confirm transfer** writes data. For a new external recipient, confirmation saves the beneficiary and transfer atomically. The receipt shows the committed transfer ID and UTC completion time, with links to activity and accounts.
